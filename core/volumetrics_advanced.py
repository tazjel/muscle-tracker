...
def slice_volume_estimate(contour: np.ndarray, pixels_per_cm: float,
    ...
    h_img = y_max - y_min + 1
    w_img = int(x_vals.max()) - int(x_vals.min()) + 1
    offset_x = int(x_vals.min())
    offset_y = y_min
    
    mask = np.zeros((h_img + 1, w_img + 1), dtype=np.uint8)
...
def compare_volume_models(contour: np.ndarray, pixels_per_cm: float) -> dict:
    ...
    if contour is not None and len(contour) > 0 and pixels_per_cm > 0:
        pts = contour.reshape(-1, 2)
        x_vals = pts[:, 0]
        y_vals = pts[:, 1]
        width_px = x_vals.max() - x_vals.min()
        height_px = y_vals.max() - y_vals.min()
        width_cm = width_px / pixels_per_cm
